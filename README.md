...
ログイン前の無料診断は最大10ページ、ログイン後の有料サイト分析は最大300ページを診断します。巡回時はSEO診断に不要な画像・動画・フォントの取得を省略し、通信量とメモリ使用量を抑えます。

Web版の診断はバックグラウンドで実行されます。画面遷移せずに診断状況を確認し、完了するとMarkdownレポートを解析して、見出しや表として同じページ内に表示します。公開診断のレポートは完了から約1時間、実行したサーバーのメモリ上に保持されます。ログイン後のサイト分析はジョブとレポートをデータベースへ保存します。

ログイン前の無料診断はトップページ（`/`）で提供します。ログイン後はダッシュボードから`/dashboard/audit`を開き、アカウントに登録したサイトを分析します。管理画面の診断はPostgreSQLのキューから専用ワーカーが実行し、ジョブ、レポート、ページ、診断項目を保存します。結果は実行したユーザーだけが取得できます。
