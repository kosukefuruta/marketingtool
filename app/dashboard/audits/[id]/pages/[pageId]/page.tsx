...
export default async function AuditPageDetail({ params, searchParams }: {
  ...
  const [[page], checks] = await Promise.all([
    db.select().from(auditPage).where(and(eq(auditPage.id, pageId), eq(auditPage.jobId, id))).limit(1),
    db.select().from(auditCheck).where(and(eq(auditCheck.jobId, id), eq(auditCheck.pageId, pageId)))
      .orderBy(asc(auditCheck.position), asc(auditCheck.id)),
  ])
  if (!page) notFound()

  return <div className="stack">
    <div><h1>{page.title || "タイトルなし"}</h1><p className="url-text">{page.url}</p></div>
    ...
    <Link href={`/dashboard/audits/${id}${fromPage > 1 ? `?page=${fromPage}` : ""}`}>ページ一覧へ戻る</Link>
  </div>
}
