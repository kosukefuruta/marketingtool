...
export default async function SiteAuditsPage({ params, searchParams }: { params: Promise<{ siteId: string }>; searchParams: Promise<{ page?: string }> }) {
  ...
  const [jobs, [activeJob]] = await Promise.all([
    ...
    paid ? db.select({ id: auditJob.id }).from(auditJob).where(and(
      eq(auditJob.siteId, siteId), eq(auditJob.userId, current.user.id), inArray(auditJob.status, ["queued", "running"]),
    )).limit(1) : [],
  ])
  const hasNextPage = jobs.length > pageSize
  const visibleJobs = jobs.slice(0, pageSize)
  const base = `/dashboard/sites/${siteId}/audits`
