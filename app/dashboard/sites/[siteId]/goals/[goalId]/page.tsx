...
export default async function GoalDetailPage({ params }: { params: Promise<{ siteId: string; goalId: string }> }) {
  ...
  const [[item], [registeredSite]] = await Promise.all([
    db.select().from(goal).where(and(eq(goal.id, goalId), eq(goal.siteId, siteId), eq(goal.userId, current.user.id))).limit(1),
    db.select({ category: site.category }).from(site).where(and(eq(site.id, siteId), eq(site.userId, current.user.id))).limit(1),
  ])
  if (!item || !isGoalMetric(item.metric)) notFound()
  ...
  const scenarios = buildGoalScenarios(item.metric, item.targetValue, category)
  return <div className="stack">
    ...
    <section className="card stack">
      ...
      <div className="summary-grid">
        ...
        <div><span>現在値</span><strong>{item.baselineValue === null ? "未取得" : formatGoalValue(item.metric, item.baselineValue)}</strong></div>
      </div>
    </section>
    ...
    {scenarios.length > 0 && <section className="card stack">
      <div><h2>達成シナリオ</h2><p className="muted">実績がない期間の初期仮定です。実測値を取得できたら自動的に置き換えます。</p></div>
